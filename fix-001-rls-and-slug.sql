-- ============================================
-- FIX 001: RLS recursion + 10-digit box slug
...
-- ============================================
-- BOXES — slug is now a 10-digit number
-- ============================================
create or replace function public.generate_box_slug()
returns text as $$
declare
  new_slug text;
  slug_exists boolean;
begin
  loop
    ...
    select exists(select 1 from public.boxes where slug = new_slug) into slug_exists;
...
create table public.boxes (
  ...
  name text not null,
  slug text not null unique default public.generate_box_slug(),
...
create policy "Box owner can update box"
  on public.boxes for update using (owner_id = auth.uid());

create policy "Authenticated users can create boxes"
  on public.boxes for insert with check (auth.uid() = owner_id);
...
-- Security definer helpers (avoids infinite recursion in box_members RLS)
...
-- Boxes SELECT policy (now that helpers exist)
create policy "Box members can view their boxes"
  on public.boxes for select
  using (id in (select public.get_user_box_ids(auth.uid())));
...
create policy "Box admins can create channels"
  ...
  with check (box_id in (select public.get_user_admin_box_ids(auth.uid())));

create policy "Box admins can update channels"
  on public.channels for update
  using (box_id in (select public.get_user_admin_box_ids(auth.uid())));
